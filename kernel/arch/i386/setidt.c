...
void encode_idt(uint64_t *idtp, uint32_t offset, uint16_t segment, uint8_t gate, uint8_t dpl, uint8_t present)
{
    if(gate > 0xf)
    {
        printf("Invalid gate value: 0x%x\n", gate);
    ...
    if(present > 1)
    {
        printf("Invalid present value: 0x%x\n", dpl);
        printf("%x %x %u %u %u %u\n", (uint32_t)idtp, offset, segment, gate, dpl, present);
        return;
    }
    printf("%x %x %x %x %x %x\n", (uint32_t)idtp, offset, segment, gate, dpl, present);
    ...
    printf("Setting at address: 0x%x\n", idtp);
    printf("or:                 0x%x\n", descriptor);
    printf("Second dword:       0x%x\n", descriptor+4);
...
{
    printf("[setup_idt] test_interrupt: 0x%x\n", test_interrupt);
